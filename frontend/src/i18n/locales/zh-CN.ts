export default {
  ...
  platform: {
    subtitle: "大模型驱动的企业级知识智能体",
    description: "Wiki 知识库、RAG 检索、智能体推理，让文档真正被理解和运用",
    rag: "RAG 增强生成",
    agent: "知识Agent",
    wiki: "Wiki 知识库",
    hybridSearch: "混合检索",
  ...
  integrations: {
    ...
    api: {
      ...
      signedFlowDetail:
        "HMAC 密钥仅保存在 XinWiki 与你的可信后端，切勿放入请求或下发给浏览器。请求头里传的是用该密钥签出的 JWT 字符串（每次签发或过期后会变），不是密钥本身。JWT 须包含 sub（外部用户 ID）、tenant_id、aud=weknora、exp（有效期不超过 24 小时）。",
      ...
      hmacSecret: "HMAC 密钥",
      hmacSecretDesc: "与 XinWiki 保存的同一份密钥；仅用于后端签发 JWT，不要作为请求头发送。",
    ...
    chrome: {
      title: "知识管理助手",
      subtitle:
        "配合 XinWiki 自建服务使用：在任意网页侧边栏提问、剪藏内容、Markdown 速记，将浏览中的知识沉淀到你的知识库。",
      ...
      steps: {
        ...
        port: {
          title: "桌面版配置固定端口（推荐）",
          desc: "使用 XinWiki 桌面版时，在 API 信息中设置固定端口（如 37841），避免每次启动后地址变化导致插件断连。",
        },
        install: {
          title: "安装 Chrome 插件",
    ...
    claw: {
      title: "XinWiki Skill",
      subtitle:
        "通过 XinWiki REST API 导入文档并执行混合检索（向量 + 关键词）。适用于上传文件/URL/Markdown 到知识库、跨库检索与浏览知识内容。",
      ...
      installCtaHint: "安装 XinWiki Skill · 将在新标签页打开",
